...
定义 description 触发评测用例。位置：skill 目录下的 `evals/trigger-evals.json`。

这个文件专供 `scripts/run_eval.py` 和 `scripts/run_loop.py` 使用，不替代 `evals/evals.json` 的人工评审用例。

```json
[
  {
    "query": "User's trigger-routing prompt",
    "should_trigger": true
  },
  {
    "query": "Irrelevant one-off task",
...
**字段说明：**
- `query`：用于测试 Skill 是否应被触发的用户原话
- `should_trigger`：布尔值，`true` 表示应触发当前 skill，`false` 表示不应触发
