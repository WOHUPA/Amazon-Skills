...
- 主壳识别与主题 CSS 同时兼容旧版 `main.main-surface` 和 Codex 26.727 的稳定 `[data-app-shell-main-surface]` 标记，避免主题切换在连接阶段超时。
- 移除会把离屏通知横幅误判为任务右栏的宽泛 `aside` 选择器，避免主题已注入后仍被严格验证错误回滚。
- 新增 Codex `26.727.4816.0` / `index-ChJf0nR3` 任务页契约回归，并修正 .NET 引擎仍上报 2.7.5 的版本常量。

### AI 主题导入简化

- AI 页面不再要求把 Studio 已保存的候选图与配方重复导入；生成完成后直接执行“一键导入主题库”，再由用户单独预览和应用。
- 主题生成器默认在 Theme Pack 目录同级输出同名 `.codextheme` Bundle；双击该文件或拖入 Studio 即可走现有的校验与确认导入流程。
- 需要只保留目录产物时，可显式使用 `--no-bundle`。

### 2.7.2 宿主适配与大背景修复

- 新增 Codex `26.721.4979.0` 精确宿主适配；窗口、主壳、侧栏、任务时间线和 composer 的稳定结构标记已重新采集。
- 修复多 MB 背景 data URL 被 Chromium 静默丢弃的问题：运行时改用短 blob URL 绑定并增加资产审计，首页/任务页背景缺失时应用不再报成功。
- 未收录的新 Codex 宿主版本仅允许无背景、无语义图标的纯配色主题使用 `native` 兼容模式；需要场景绑定的 Theme Pack 必须通过严格宿主与资产证据，杜绝“显示已应用但画面没变化”。

### 2.7.1 主题切换与中文体验修复

- 未收录的新 Codex 宿主版本在 `native` 布局、基础注入、几何与对比度均通过时允许以兼容模式应用；严格验证和发布门禁仍保持 `BLOCKED`。
- 系列创建只需输入显示名称，支持中文；安全内部 ID 由客户端自动生成。
- Node 子进程输出固定按 UTF-8 解码，运行时失败改为简短可读信息；中文图片路径与背景校验错误不再乱码。
