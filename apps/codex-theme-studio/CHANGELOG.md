...
- 新增 RuntimeSupervisor，CDP 身份仍有效时自动恢复 watcher；需要结束并重启 Codex 时必须先由用户确认。
- watcher 改为 CDP Target 事件和主题文件变更通知驱动，30 秒仅执行轻量健康检查；渲染器日常 DOM 变化只做增量同步。
- 新增 `.codextheme` Bundle v1、严格安全验证、整包事务和文件关联；导入后不自动激活。
- 新增 Catalog v1、基础/斗破内置系列、虚拟系列和本地系列管理，保留用户自定义命名与归属。
- 主题列表采用 18 卡片窗口化渲染，图片改用限制解码尺寸的异步缩略图与 LRU 缓存；相同首页/任务页背景在 CDP 载荷中只编码一次。
- 运行页两个板块改为等高拉伸布局，并显示正常、自愈中、需要重启、暂停、离线与故障状态。
- Generator 与 Selector 接入 Bundle/原生引擎契约；移除被原生客户端替代的 PowerShell GUI/CLI。
