...
function Enter-DreamSkinOperationLock {
  ...
  $mutex = [System.Threading.Mutex]::new($false, "Local\CodexThemeStudio.$sid.Operation")
  $acquired = $false
  try {
  ...
  if (-not $acquired) {
    $mutex.Dispose()
    throw 'Another Codex Theme Studio install, start, restore, or verify operation is already running.'
  }
  return $mutex
}
...
function Get-DreamSkinRuntimeEnginePaths {
  ...
  return [pscustomobject]@{
    ...
    Cli = Join-Path $scripts 'theme-studio.ps1'
  }
}
...
function Install-DreamSkinRuntimeEngine {
  ...
  $required = @(
    ...
    'assets\host-adapters.json',
    'assets\renderer-inject.js',
    'presets\immersive-dark\theme.json',
    'presets\clear-light\theme.json',
    'presets\obsidian-gold\theme.json',
    'scripts\common-windows.ps1',
    'scripts\compile_native_theme.py',
    ...
    'scripts\theme-studio.ps1',
    'scripts\theme_common.py',
    'scripts\validate_theme_v2.py',
    'scripts\tray-dream-skin.ps1',
    'scripts\verify-dream-skin.ps1',
    'references\visual-regression-matrix.json',
    'assets\create-theme-prompt.txt',
    'assets\studio-version.txt',
    'scripts\desktop-bootstrap.ps1'
  )
  ...
  $runtimeDirectories = @('assets', 'scripts', 'presets', 'references')
...
function Initialize-DreamSkinPackageLauncher {
  if ('CodexThemeStudio.PackageLauncher' -as [type]) { return }
  Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

namespace CodexThemeStudio {
  [Flags]
...
function Start-DreamSkinCodex {
  ...
  $processId = [CodexThemeStudio.PackageLauncher]::Launch($appUserModelId, $argumentLine)
...
function Initialize-DreamSkinTcpTable {
  if ('CodexThemeStudio.TcpTable' -as [type]) { return }
  Add-Type -TypeDefinition @'
using System;
...
namespace CodexThemeStudio {
  public sealed class TcpListenerInfo {
  ...
  public static class TcpTable {
    ...
    public static TcpListenerInfo[] GetListeners(int port) {
      var listeners = new List<TcpListenerInfo>();
      ReadTable(AfInet, 24, port, listeners);
      ReadTable(AfInet6, 56, port, listeners);
    ...
    private static void ReadTable(
      ...
      int rowSize,
      int requestedPort,
      List<TcpListenerInfo> listeners) {
      int size = 0;
      ...
      try {
        ...
        for (int index = 0; index < count; index++) {
          ...
          int localPortOffset = addressFamily == AfInet ? 8 : 20;
          int ownerPidOffset = addressFamily == AfInet ? 20 : 52;
          int localPort = DecodePort(unchecked((uint)Marshal.ReadInt32(row, localPortOffset)));
          if (localPort != requestedPort) continue;

          string localAddress;
          if (addressFamily == AfInet) {
            byte[] addressBytes = BitConverter.GetBytes(Marshal.ReadInt32(row, 4));
            localAddress = new IPAddress(addressBytes).ToString();
          } else {
            byte[] addressBytes = new byte[16];
            Marshal.Copy(row, addressBytes, 0, addressBytes.Length);
            long scopeId = unchecked((uint)Marshal.ReadInt32(row, 16));
            localAddress = new IPAddress(addressBytes, scopeId).ToString();
          }

          listeners.Add(new TcpListenerInfo {
            LocalAddress = localAddress,
            LocalPort = localPort,
            OwningProcess = Marshal.ReadInt32(row, ownerPidOffset)
          });
        }
      } finally {
    ...
    private static int DecodePort(uint value) {
      return unchecked((ushort)IPAddress.NetworkToHostOrder(unchecked((short)(value & 0xffff))));
    }
  }
}
'@
...
function Get-DreamSkinPortListeners {
  ...
  try {
    ...
    return @([CodexThemeStudio.TcpTable]::GetListeners($Port))
  } catch {
    # NOTE: 原生 TCP 表避免托盘每次打开都触发两次高延迟的 Get-NetTCPConnection；
    # 仅在旧版或受限 Windows 环境无法加载系统 API 时回退到原有安全检查。
    if (-not (Get-Command Get-NetTCPConnection -ErrorAction SilentlyContinue)) {
      throw 'Windows TCP listener ownership cannot be verified.'
    }
...
function Test-DreamSkinCodexPortOwner {
  ...
  foreach ($listener in $listeners) {
    ...
    try {
      # NOTE: 端口表已经给出唯一 PID，直接读取进程路径与 CIM 查询具有相同的身份校验结果，
      # 但不会在托盘菜单打开时触发高延迟的 WMI 往返。
      $process = Get-Process -Id ([int]$listener.OwningProcess) -ErrorAction Stop
      $processPath = if ($process.Path) { "$($process.Path)" } else { "$($process.MainModule.FileName)" }
    } catch {
      $processPath = $null
    }
...
function Stop-DreamSkinRecordedInjector {
  ...
  while ((Get-CimInstance Win32_Process -Filter "ProcessId = $processId" -ErrorAction SilentlyContinue) -and
...
function Confirm-DreamSkinRestart {
  ...
  return $shell.Popup($Message, 0, 'Codex Theme Studio', 52) -eq 6
}
