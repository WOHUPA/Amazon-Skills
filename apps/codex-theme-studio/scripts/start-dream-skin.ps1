...
try {
  ...
  Ensure-DreamSkinManagedDirectory -Path $LogsRoot -Root $StateRoot
  $StdoutPath = Join-Path $LogsRoot 'injector.log'
  $StderrPath = Join-Path $LogsRoot 'injector-error.log'
  $VerifyPath = Join-Path $LogsRoot 'verify.log'
  ...
  try {
    $injectorArgs = @((ConvertTo-DreamSkinProcessArgument -Value $Injector), '--watch', '--port', "$Port",
      ...
      (ConvertTo-DreamSkinProcessArgument -Value $themePaths.PauseFile), '--codex-version', $codex.Version)
    ...
    $activeThemeId = $null
    $previousThemeId = if ($previousState -and $previousState.previousThemeId) { "$($previousState.previousThemeId)" } else { $null }
    try { $activeThemeId = "$( (Read-DreamSkinTheme -ThemeDirectory $themePaths.Active -SkipImageMetadata).Theme.id )" } catch {}
    $previousThemePath = Join-Path $StateRoot 'backups\previous-theme'
    if (Test-Path -LiteralPath (Join-Path $previousThemePath 'theme.json') -PathType Leaf) {
      try { $previousThemeId = "$( (Read-DreamSkinTheme -ThemeDirectory $previousThemePath -SkipImageMetadata).Theme.id )" } catch {}
    }
  ...
  Write-Host "Codex Theme Studio is active on verified loopback port $Port."
} finally {
