...
$verifyExitCode = 1
try {
  $StatePath = Join-Path $env:LOCALAPPDATA 'CodexThemeStudio\state.json'
  ...
  $verifyMode = if ($ExpectRemoved) { '--verify-removed' } else { '--verify' }
  $arguments = @($injector, $verifyMode, '--port', "$Port", '--browser-id', $cdpIdentity.BrowserId,
    '--timeout-ms', '30000')
