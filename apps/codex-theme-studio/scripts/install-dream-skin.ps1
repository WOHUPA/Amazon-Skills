...
try {
  ...
  if (Test-DreamSkinTrayActive) {
    throw 'Exit the Codex Theme Studio tray before reinstalling so every shortcut can move to the new runtime safely.'
  }
  ...
  if (-not $NoShortcuts) {
    ...
    foreach ($folder in @($desktop, $startMenu)) {
      ...
      $shortcut.Description = 'Launch the official Codex app with Codex Theme Studio'
      $shortcut.Save()
    }
    ...
    foreach ($folder in @($desktop, $startMenu)) {
      $tray = $shell.CreateShortcut((Join-Path $folder 'Codex Theme Studio - Tray.lnk'))
      ...
      $tray.Description = 'Open Codex Theme Studio status and theme controls in the system tray'
      $tray.Save()
    }
  ...
  if ($NoShortcuts) {
    Write-Host "Codex Theme Studio base theme installed at $($engine.Root). Run $($engine.Start) to launch it."
  } else {
    Write-Host 'Codex Theme Studio installed. The launch shortcut asks before restarting an open Codex window.'
  }
} finally {
