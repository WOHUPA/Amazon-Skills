...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-dream-skin.ps1` once. The installer preserves the user's native appearance settings, seeds the Xiao Yan Inferno theme, copies the runtime to `%LOCALAPPDATA%\CodexThemeStudio\engine`, and creates launch/restore/tray shortcuts that do not depend on the source checkout.
2. Use the `Codex Theme Studio` shortcut, or run `%LOCALAPPDATA%\CodexThemeStudio\engine\scripts\start-dream-skin.ps1`. The shortcut asks before restarting an already-open Codex app; CLI callers must explicitly add `-RestartExisting`.
...
- Paint one continuous 16:9 wallpaper across the full Codex window without changing Codex geometry. The sidebar, main area, header, composer, cards, typography, spacing, and responsive breakpoints must retain their native layout.
- Prefer revealing that continuous wallpaper wherever readability allows. The left sidebar and right-side chrome must share one clear alpha-background material without backdrop blur; composer, dialogs, cards, and secondary surfaces use a slightly stronger plain-alpha material instead of opaque fills.
...
- Theme Codex's real project-selector toolbar only through colors, background material, border color, radius, shadow, or blur; never resize, move, hide, reparent, or replace it.
- Keep decorative layers `pointer-events: none` and keep real buttons, navigation, and composer above them.
- Treat native layout as immutable. CSS targeting Codex nodes must not set positioning, stacking, display, visibility, overflow, dimensions, spacing, flex/grid, transforms, font metrics, opacity, pointer events, or border widths. Theme packages cannot contain CSS, style, layout, or selector overrides.
- Never make native containers transparent with the `opacity` property. Use background alpha alone for shared chrome and composer/dialog surfaces, preserving the wallpaper's original detail while text, icons, focus indicators, and hit targets stay fully opaque.
- Keep the background on the existing window and surface paint layers. Never create a new stacking context on native Codex containers merely to place artwork behind them.
...
- Keep the injection daemon running for navigation/reload resilience. Its state and logs live under `%LOCALAPPDATA%\CodexThemeStudio`.
- Treat `state.json` as a previous-session record, not a liveness signal. Tray status and theme switches must validate the registered package, active loopback endpoint, and saved Browser ID; theme changes must re-enter the start/apply path so a dead watcher is replaced or a normal Codex launch receives an explicit restart prompt.
...
- The active theme, saved themes, imported images, pause marker, and tray controls live under `%LOCALAPPDATA%\CodexThemeStudio`. Reject empty or over-16 MB images before copying or encoding them.
...
- Treat `%LOCALAPPDATA%\CodexThemeStudio\engine` as an installer-managed runtime. Exit the Dream Skin tray before reinstalling so the installer can replace that runtime atomically and update every shortcut to the same copy.
...
- `assets/dream-skin.css`: layout-neutral color, material, and background layer.
- `assets/renderer-inject.js`: idempotent DOM integration and cleanup.
- `presets/preset-xiao-yan-inferno/`: pure 2560 × 1440 bundled wallpaper and theme contract seeded as the default and as a saved theme; it contains no Codex UI.
