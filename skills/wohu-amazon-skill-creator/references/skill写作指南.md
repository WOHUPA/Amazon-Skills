...
| 层级 | 什么时候加载 | 大小建议 |
|------|--------------|----------|
| **1. 元数据**（name + description） | 永远在 Codex 上下文里 | ~100 词 |
...
这是 Codex 决定要不要调用这个 skill 的**主要依据**。

### 反例（太保守，触发不到）

> 一个关键词自然排名分析的 skill

问题：没说什么时候用、用在什么场景、用户会说什么话触发。Codex 读完不知道该不该调用。
...
- **最后补了一句"即使没明说 skill 也要触发"**——降低漏触发概率
...
分析、诊断、监控或运营报告不能只有展示模板。先按 `references/report-editions.md` 定义数据来源表、指标字典和版本路由，再写正文结构：

```markdown
## 数据来源与指标口径
[基础/增强来源表 + 指标定义、公式、单位、粒度、字段等级和缺失行为]

## 报告版本路由
- 默认：report_edition=auto
- 基础版报告：[basic_required 范围]
- 增强版报告（部分增强）：[至少一个完整增强模块]
- 增强版报告（完整增强）：[完整增强清单全部满足]

## 模块覆盖与动作门槛
| 模块 | module_required | full_required | 缺失影响 |
```

`display_optional` 只影响展示，不得控制完整增强；单个增强模块缺失只让对应章节 `SKIP` 或动作 `MANUAL`。非报告型 Skill 不加入这些章节。

### 5.5 Examples Pattern
...
SKILL.md 里写"用户问的是 SP 就读 `references/sp.md`"。Codex 只加载需要的那份，上下文不会撑满。
...
```bash
uv run --with PyYAML python scripts/quick_validate.py <你的 skill 目录>
```
